...
impl Drop for NftRconeModule {
    fn drop(&mut self) {
        ...
        pr_info!("nft_rcone: module and mappings removed\n");
    }
}
    ...
    unsafe {
        ...
        match hooknum {
            bindings::nf_inet_hooks_NF_INET_PRE_ROUTING => {
                ...
                // Restricted Cone NAT 逻辑
                let found = mappings_lock.data.iter()
                    .find(|m| m.map_ip == dst_ip && m.map_port == dst_port && m.ext_addr == src_ip)
                    .copied();
            ...
            bindings::nf_inet_hooks_NF_INET_POST_ROUTING => {
                ...
                if !mappings_lock.data.iter().any(|m| m.int_addr == src_ip && m.int_port == src_port) {
                    ...
                    if mappings_lock.data.push(new_mapping, flags::GFP_ATOMIC).is_err() {
                        pr_warn!("nft_rcone: Failed to add mapping (OOM)\n");
                    }
                }
                bindings::_raw_spin_unlock(raw_lock);
...
extern "C" fn nft_rcone_dump(_skb: *mut bindings::sk_buff, _expr: *const bindings::nft_expr, _reset: bool) -> i32 { 0 }
extern "C" fn nft_rcone_validate(ctx: *const bindings::nft_ctx, _expr: * const bindings::nft_expr) -> i32 {
...
#[repr(C)]
struct NftRconePriv { _flags: u32, _sreg_proto_min: u8, _sreg_proto_max: u8 }

fn nft_rcone_ipv4_ops() -> bindings::nft_expr_ops {
    bindings::nft_expr_ops {
        size: core::mem::size_of::<NftRconePriv>() as u32,
        eval: Some(nft_rcone_ipv4_eval),
        init: Some(nft_rcone_init),
        destroy: Some(nft_rcone_destroy),
        dump: Some(nft_rcone_dump),
        validate: Some(nft_rcone_validate),
