...
//! Rust implementation of nftables rcone expression support.
...
impl kernel::Module for NftRconeModule {
    fn init(_module: &'static ThisModule) -> Result<Self> {
        ...
        expr_type.name = "rcone\0".as_ptr() as *const u8;
        expr_type.family = NFPROTO_IPV4 as u8;
        expr_type.ops = expr_ops_ptr as *const nft_expr_ops;
        expr_type.policy = nft_rcone_policy().as_ptr() as *const _;
        // expr_type.maxattr = NftRconeAttributes::NFTA_RCONE_MAX as u32;
        expr_type.maxattr = 3;
        ...
        pr_info!("nft_rcone: module initialized successfully\n");
        Ok(NftRconeModule {
...
impl Drop for NftRconeModule {
    fn drop(&mut self) {
        ...
        pr_info!("nft_rcone: mappings cleared\n");
    }
}
...
// 设置 NAT 范围
fn nft_rcone_set_regs(expr: *const nft_expr, regs: *const nft_regs, range: &mut nf_nat_range2) {
    // unsafe {
    //     let priv = bindings::nft_expr_priv(expr) as *const NftRcone;
...
// IPv4 rcone 评估函数
extern "C" fn nft_rcone_ipv4_eval(expr: *const nft_expr, regs: *mut nft_regs, pkt: *const nft_pktinfo) {
    let mut range = nf_nat_range2::default();
    unsafe {
        // nft_rcone_set_regs(expr, regs, &mut range);
        ...
        match hooknum {
            nf_inet_hooks_NF_INET_PRE_ROUTING => {
                ...
                // pr_err!("nft_rcone: INBOUND DNAT from {}:{} to {}:{}\n",
            ...
            nf_inet_hooks_NF_INET_POST_ROUTING => {
                ...
                // pr_err!("nft_rcone: OUTBOUND SNAT from {}:{} to {}:{}\n",
                //     Ipv4Addr::from_bits(src_ip), src_port,
                //     Ipv4Addr::from_bits(dst_ip), dst_port);
...
// 销毁函数
extern "C" fn nft_rcone_destroy(ctx: *const nft_ctx, _expr: *const nft_expr) {
}

// Netlink 属性策略
fn nft_rcone_policy() -> [nla_policy; 4] {
...
extern "C" fn nft_rcone_validate(ctx: *const nft_ctx, expr: * const nft_expr ) -> i32 {
    // pr_warn!("{} message (level {}) with nft_rcone_validate\n", "Warning", 4);
    let err = unsafe {
        nft_chain_validate_dependency((*ctx).chain,nft_chain_types_NFT_CHAIN_T_NAT)
    };
    if err <0 {
        pr_info!("nft_rcone: NAT chain not found\n");
        return err;
    }
	let err = unsafe {
...
extern "C" fn nft_rcone_dump(skb: *mut sk_buff, expr: *const nft_expr, reset: bool_) ->i32 {
    // pr_warn!("{} message (level {}) with nft_rcone_dump\n", "Warning", 4);
    0
}
...
// IPv4 表达式操作
fn nft_rcone_ipv4_ops() -> nft_expr_ops {
    nft_expr_ops {
        // type_: unsafe { &NFT_RCONE_IPV4_TYPE as *const _ },

        //设置为0将会很坑！！！
        size: (core::mem::size_of::<nft_rcone>() as u32) + (core::mem::size_of::<nft_expr>() as u32),
        // size: 0,
        eval: Some(nft_rcone_ipv4_eval),
        init: Some(nft_rcone_init),
        destroy: Some(nft_rcone_destroy),
        dump: Some(nft_rcone_dump),
        validate: Some(nft_rcone_validate), // 未实现
        ..nft_expr_ops::default()
    }
}

// 静态表达式操作和类型定义
// static NFT_RCONE_IPV4_OPS: nft_expr_ops = nft_rcone_ipv4_ops();

// static mut NFT_RCONE_IPV4_TYPE: nft_expr_type = nft_expr_type {
//     family: NFPROTO_IPV4 as u32,
//     name: b"rcone\0".as_ptr() as *const i8,
//     ops: unsafe { &NFT_RCONE_IPV4_OPS as *const _ },
//     policy: nft_rcone_policy().as_ptr() as *const _,
//     maxattr: NFTA_RCONE_MAX as u32,
