...
static void netlink_parse_rcone(struct netlink_parse_ctx *ctx,
			       const struct location *loc,
			       const struct nftnl_expr *nle)
{
	enum nft_registers reg1, reg2;
	...
	if (nftnl_expr_is_set(nle, NFTNL_EXPR_RCONE_FLAGS))
		flags = nftnl_expr_get_u32(nle, NFTNL_EXPR_RCONE_FLAGS);

	stmt = nat_stmt_alloc(loc, NFT_NAT_RCONE);
	stmt->nat.flags = flags;

	reg1 = netlink_parse_register(nle, NFTNL_EXPR_RCONE_REG_PROTO_MIN);
	if (reg1) {
		proto = netlink_get_register(ctx, loc, reg1);
		if (proto == NULL) {
			netlink_error(ctx, loc,
				      "rcone statement has no proto expression");
			goto out_err;
		}
		expr_set_type(proto, &inet_service_type, BYTEORDER_BIG_ENDIAN);
		stmt->nat.proto = proto;
	...
	reg2 = netlink_parse_register(nle, NFTNL_EXPR_RCONE_REG_PROTO_MAX);
	if (reg2 && reg2 != reg1) {
		proto = netlink_get_register(ctx, loc, reg2);
		if (proto == NULL) {
			netlink_error(ctx, loc,
...
static void netlink_parse_prcone(struct netlink_parse_ctx *ctx,
			       const struct location *loc,
			       const struct nftnl_expr *nle)
{
	enum nft_registers reg1, reg2;
	...
	if (nftnl_expr_is_set(nle, NFTNL_EXPR_PRCONE_FLAGS))
		flags = nftnl_expr_get_u32(nle, NFTNL_EXPR_PRCONE_FLAGS);

	stmt = nat_stmt_alloc(loc, NFT_NAT_PRCONE);
	stmt->nat.flags = flags;

	reg1 = netlink_parse_register(nle, NFTNL_EXPR_PRCONE_REG_PROTO_MIN);
	if (reg1) {
		proto = netlink_get_register(ctx, loc, reg1);
		if (proto == NULL) {
			netlink_error(ctx, loc,
				      "prcone statement has no proto expression");
			goto out_err;
		}
		expr_set_type(proto, &inet_service_type, BYTEORDER_BIG_ENDIAN);
		stmt->nat.proto = proto;
	...
	reg2 = netlink_parse_register(nle, NFTNL_EXPR_PRCONE_REG_PROTO_MAX);
	if (reg2 && reg2 != reg1) {
		proto = netlink_get_register(ctx, loc, reg2);
		if (proto == NULL) {
			netlink_error(ctx, loc,
...
static const struct expr_handler netlink_parsers[] = {
	...
	{ .name = "rcone",	.parse = netlink_parse_rcone },
	{ .name = "prcone",	.parse = netlink_parse_prcone },
