...
/*
 * RCONE_HKEY generates u32 hash value
 ...
 */
#define RCONE_HKEY(dataptr, datatypelen, initval, htable_bits)			\
({								\
...
static DEFINE_SPINLOCK(rconenat_lock);
...
static DEFINE_SPINLOCK(rconenat6_lock);
#endif
...
static char *rcone_nf_ct_stringify_tuple6(const struct nf_conntrack_tuple
					     *t);
#endif
/* non-atomic: can only be called serially within lock zones. */
...
#if IS_ENABLED(CONFIG_NF_NAT_IPV6) || (IS_ENABLED(CONFIG_IPV6) && LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
/* non-atomic: can only be called serially within lock zones. */
static char *rcone_nf_ct_stringify_tuple6(const struct nf_conntrack_tuple *t)
{
...
EXPORT_SYMBOL_GPL(nf_nat_rcone_dying_tuple_list_add);
...
void nf_nat_rcone_handle_dying_tuples(void)
{
	...
	spin_lock_bh(&rconenat6_lock);
#endif

	spin_lock_bh(&rconenat_lock);
	spin_lock_bh(&dying_tuple_list_lock);
	...
	list_for_each_safe(iter, tmp, &dying_tuple_list) {
		...
		if (ct_tuple->src.l3num == PF_INET6) {
			...
			if (mapping6 == NULL) {
				...
				if (mapping6 != NULL) {
					pr_debug
					    ("nf_nat_rcone_handle_dying_tuples(): INBOUND dying conntrack at ext port %d\n",
					     mapping6->port);
				}
			} else {
				pr_debug
				    ("nf_nat_rcone_handle_dying_tuples(): OUTBOUND dying conntrack at ext port %d\n",
				     mapping6->port);
			}
			...
			list_for_each_safe(iter_2, tmp_2, &mapping6->original_tuple_list) {
				...
				if (nf_ct_tuple_equal(&original_tuple_item->tuple, &(item->tuple_original))) {
					pr_debug
					    ("nf_nat_rcone_handle_dying_tuples(): tuple %s expired. free this tuple.\n",
					     rcone_nf_ct_stringify_tuple6(&original_tuple_item->tuple));
			...
			pr_debug
			    ("nf_nat_rcone_handle_dying_tuples(): refer_count for mapping at ext_port %d is now %d\n",
			     mapping6->port, mapping6->refer_count);
			if (mapping6->refer_count <= 0) {
				pr_debug
				    ("nf_nat_rcone_handle_dying_tuples(): kill expired mapping at ext port %d\n",
				     mapping6->port);
...
next:
	...
	spin_unlock_bh(&dying_tuple_list_lock);
	spin_unlock_bh(&rconenat_lock);
#if IS_ENABLED(CONFIG_NF_NAT_IPV6) || (IS_ENABLED(CONFIG_IPV6) && LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
	spin_unlock_bh(&rconenat6_lock);
#endif
}

EXPORT_SYMBOL_GPL(nf_nat_rcone_handle_dying_tuples);
...

{
	...
	hash_src = RCONE_HKEY(int_addr, sizeof(union nf_inet_addr), (u32) int_port, HASHTABLE_BUCKET_BITS);
					    ...
					    const uint16_t int_port, const uint16_t port, const __be32 addr,const __be32 ext_addr, const uint16_t ext_port)
{
	...
	hash_src = RCONE_HKEY(&int_addr, sizeof(__be32), (u32) int_port, HASHTABLE_BUCKET_BITS);
						    ...
						    *ext_ip)
{
	...
	u32 hash_src = RCONE_HKEY(src_ip, sizeof(union nf_inet_addr), (u32) src_port, HASHTABLE_BUCKET_BITS);
...
static struct nat_mapping *get_mapping_by_int_src(const __be32 src_ip, const uint16_t src_port, const __be32 ext_ip)
{
	...
	u32 hash_src = RCONE_HKEY(&src_ip, sizeof(__be32), (u32) src_port, HASHTABLE_BUCKET_BITS);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 3, 0)
	j = RCONE_HKEY(src, sizeof(union nf_inet_addr),
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 3, 0)
	j = RCONE_HKEY(&src, sizeof(__be32), range->flags & NF_NAT_RANGE_PERSISTENT ? 0 : dst ^ zone->id,
			  HASHTABLE_BUCKET_BITS);
	//j = jhash_1word((u32) src, range->flags & NF_NAT_RANGE_PERSISTENT ? 0 : dst ^ zone->id);
#else
	j = RCONE_HKEY(&src, sizeof(__be32), range->flags & NF_NAT_RANGE_PERSISTENT ? 0 : dst ^ zone,
			  HASHTABLE_BUCKET_BITS);
...
EXPORT_SYMBOL_GPL(nf_nat_rcone_destroy_mappings);
...
/* NFPROTO specific def */
	...
	if (protonum != IPPROTO_UDP) {
		...
		// For other protos RCONENAT is equivalent to MASQUERADE.
		return ret;
	}
	...
	if (nfproto == NFPROTO_IPV4) {
		...
		pr_warn("<INBOUND DNAT> %s ==> [%pI6c]:%d\n",
			 rcone_nf_ct_stringify_tuple6(ct_tuple_origin), &mapping_6->int_addr, mapping_6->int_port);
	}
					      ...
					      const struct net_device *out)
{
	...
	if (nfproto == NFPROTO_IPV4) {
		...
		pr_warn("test1 <OUTBOUND SNAT> %s ==> %d\n", rcone_nf_ct_stringify_tuple6(ct_tuple_origin), port);
	}
...
EXPORT_SYMBOL_GPL(nf_nat_rcone_ipv4);
...
EXPORT_SYMBOL_GPL(nf_nat_rcone_ipv6);
