...
/*
 * PRCONE_HKEY generates u32 hash value
 ...
 */
#define PRCONE_HKEY(dataptr, datatypelen, initval, htable_bits)			\
({								\
...
static DEFINE_SPINLOCK(prconenat_lock);
...
static DEFINE_SPINLOCK(prconenat6_lock);
#endif
...
static char *prcone_nf_ct_stringify_tuple6(const struct nf_conntrack_tuple
					     *t);
#endif
/* non-atomic: can only be called serially within lock zones. */
...
static struct nat_mapping6 *allocate_mapping6(const union nf_inet_addr
					      *int_addr,
					      ...
					      const uint16_t port, const union nf_inet_addr *addr,const union nf_inet_addr *ext_addr,const uint16_t ext_port);
#endif
static struct nat_mapping *allocate_mapping(const __be32 int_addr,
	const uint16_t int_port, const uint16_t port, const __be32 addr,const __be32 ext_addr, const uint16_t ext_port);
...
#if IS_ENABLED(CONFIG_NF_NAT_IPV6) || (IS_ENABLED(CONFIG_IPV6) && LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
/* non-atomic: can only be called serially within lock zones. */
static char *prcone_nf_ct_stringify_tuple6(const struct nf_conntrack_tuple *t)
{
...
EXPORT_SYMBOL_GPL(nf_nat_prcone_dying_tuple_list_add);
...
void nf_nat_prcone_handle_dying_tuples(void)
{
	...
	spin_lock_bh(&prconenat6_lock);
#endif

	spin_lock_bh(&prconenat_lock);
	spin_lock_bh(&dying_tuple_list_lock);
	...
	list_for_each_safe(iter, tmp, &dying_tuple_list) {
		...
		if (ct_tuple->src.l3num == PF_INET6) {
			...
			if (mapping6 == NULL) {
				...
				if (mapping6 != NULL) {
					pr_debug
					    ("nf_nat_prcone_handle_dying_tuples(): INBOUND dying conntrack at ext port %d\n",
					     mapping6->port);
				}
			} else {
				pr_debug
				    ("nf_nat_prcone_handle_dying_tuples(): OUTBOUND dying conntrack at ext port %d\n",
				     mapping6->port);
			}
			...
			list_for_each_safe(iter_2, tmp_2, &mapping6->original_tuple_list) {
				...
				if (nf_ct_tuple_equal(&original_tuple_item->tuple, &(item->tuple_original))) {
					pr_debug
					    ("nf_nat_prcone_handle_dying_tuples(): tuple %s expired. free this tuple.\n",
					     prcone_nf_ct_stringify_tuple6(&original_tuple_item->tuple));
			...
			pr_debug
			    ("nf_nat_prcone_handle_dying_tuples(): refer_count for mapping at ext_port %d is now %d\n",
			     mapping6->port, mapping6->refer_count);
			if (mapping6->refer_count <= 0) {
				pr_debug
				    ("nf_nat_prcone_handle_dying_tuples(): kill expired mapping at ext port %d\n",
				     mapping6->port);
...
next:
	...
	spin_unlock_bh(&dying_tuple_list_lock);
	spin_unlock_bh(&prconenat_lock);
#if IS_ENABLED(CONFIG_NF_NAT_IPV6) || (IS_ENABLED(CONFIG_IPV6) && LINUX_VERSION_CODE >= KERNEL_VERSION(5, 1, 0))
	spin_unlock_bh(&prconenat6_lock);
#endif
}

EXPORT_SYMBOL_GPL(nf_nat_prcone_handle_dying_tuples);
...
static struct nat_mapping6 *allocate_mapping6(const union nf_inet_addr *int_addr,
	const uint16_t int_port,const uint16_t port, 
	const union nf_inet_addr *addr,const union nf_inet_addr *ext_addr,const uint16_t ext_port)

{
	...
	p_new->ext_addr = *ext_addr;
	p_new->ext_port = ext_port;

	pr_warn("test2 int_addr=%pI6 int_port=%u port=%u addr=%pI6 ext_addr=%pI6 ext_port=%u\n",
		int_addr, int_port, port, addr, ext_addr, ext_port);

	p_new->refer_count = 0;
	(p_new->original_tuple_list).next = &(p_new->original_tuple_list);
	(p_new->original_tuple_list).prev = &(p_new->original_tuple_list);

	hash_src = PRCONE_HKEY(int_addr, sizeof(union nf_inet_addr), (u32) int_port, HASHTABLE_BUCKET_BITS);
					    ...
					    const uint16_t int_port, const uint16_t port, const __be32 addr,const __be32 ext_addr, const uint16_t ext_port)
{
	...
	hash_src = PRCONE_HKEY(&int_addr, sizeof(__be32), (u32) int_port, HASHTABLE_BUCKET_BITS);
						    ...
						    *ext_ip)
{
	...
	u32 hash_src = PRCONE_HKEY(src_ip, sizeof(union nf_inet_addr), (u32) src_port, HASHTABLE_BUCKET_BITS);
...
static struct nat_mapping *get_mapping_by_int_src(const __be32 src_ip, const uint16_t src_port, const __be32 ext_ip)
{
	...
	u32 hash_src = PRCONE_HKEY(&src_ip, sizeof(__be32), (u32) src_port, HASHTABLE_BUCKET_BITS);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 3, 0)
	j = PRCONE_HKEY(src, sizeof(union nf_inet_addr),
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 3, 0)
	j = PRCONE_HKEY(&src, sizeof(__be32), range->flags & NF_NAT_RANGE_PERSISTENT ? 0 : dst ^ zone->id,
			  HASHTABLE_BUCKET_BITS);
	//j = jhash_1word((u32) src, range->flags & NF_NAT_RANGE_PERSISTENT ? 0 : dst ^ zone->id);
#else
	j = PRCONE_HKEY(&src, sizeof(__be32), range->flags & NF_NAT_RANGE_PERSISTENT ? 0 : dst ^ zone,
			  HASHTABLE_BUCKET_BITS);
...
EXPORT_SYMBOL_GPL(nf_nat_prcone_destroy_mappings);
...
/* NFPROTO specific def */
	...
	if (protonum != IPPROTO_UDP) {
		...
		// For other protos PRCONENAT is equivalent to MASQUERADE.
		return ret;
	}
	...
	if (nfproto == NFPROTO_IPV4) {
		pr_warn("<INBOUND DNAT> %s ==> %pI4:%d\n",
			 nf_ct_stringify_tuple(ct_tuple_origin), &mapping->int_addr, mapping->int_port);
	} else if (nfproto == NFPROTO_IPV6) {
		pr_warn("<INBOUND DNAT> %s ==> [%pI6c]:%d\n",
			 prcone_nf_ct_stringify_tuple6(ct_tuple_origin), &mapping_6->int_addr, mapping_6->int_port);
	}
					      ...
					      const struct net_device *out)
{
	...
	if (nfproto == NFPROTO_IPV4) {
		...
		pr_warn("test1 <OUTBOUND SNAT> %s ==> %d\n", prcone_nf_ct_stringify_tuple6(ct_tuple_origin), port);
	}
	...
	if (nfproto == NFPROTO_IPV4) {
		...
		if (mapping_6 == NULL) {
			...
			mapping_6 = allocate_mapping6(ip_6, original_port, port, &(ct_tuple->dst).u3,&(ct_tuple_origin->dst).u3,ntohs(ct_tuple_origin->dst.u.udp.port));
...
EXPORT_SYMBOL_GPL(nf_nat_prcone_ipv4);
...
EXPORT_SYMBOL_GPL(nf_nat_prcone_ipv6);
