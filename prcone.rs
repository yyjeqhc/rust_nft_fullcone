...
impl kernel::Module for NftPrconeModule {
    fn init(_module: &'static ThisModule) -> Result<Self> {
        ...
        unsafe {
            ...
            let name = b"nft_prcone_lock\0".as_ptr();
        ...
        expr_type.name = b"prcone\0".as_ptr();
        ...
        if ret != 0 {
            pr_err!("nft_prcone: failed to register expression: {}\n", ret);
            unsafe {
                if !MAPPINGS.is_null() {
...
impl Drop for NftPrconeModule {
    fn drop(&mut self) {
        ...
        pr_info!("nft_prcone: module and mappings removed\n");
    }
}
...
extern "C" fn nft_prcone_ipv4_eval(_expr: *const bindings::nft_expr, regs: *mut bindings::nft_regs, pkt: *const bindings::nft_pktinfo) {
    unsafe {
            ...
            bindings::nf_inet_hooks_NF_INET_POST_ROUTING => {
                ...
                if !mappings_lock.data.iter().any(|m| m.int_addr == src_ip && m.int_port == src_port) {
                    let new_mapping = NatMapping {
                        ...
                        map_ip, map_port,
                        ext_addr: dst_ip, ext_port: dst_port,
                    };
                    if mappings_lock.data.push(new_mapping, flags::GFP_ATOMIC).is_err() {
                        pr_warn!("nft_prcone: Failed to add mapping (OOM)\n");
                    }
                }
                bindings::_raw_spin_unlock(raw_lock);
...
extern "C" fn nft_prcone_init(_ctx: *const bindings::nft_ctx, _expr: *const bindings::nft_expr, _tb: *const *const bindings::nlattr) -> i32 { 0 }
extern "C" fn nft_prcone_destroy(_ctx: *const bindings::nft_ctx, _expr: *const bindings::nft_expr) {}
extern "C" fn nft_prcone_dump(_skb: *mut bindings::sk_buff, _expr: *const bindings::nft_expr, _reset: bool) -> i32 { 0 }
extern "C" fn nft_prcone_validate(ctx: *const bindings::nft_ctx, _expr: * const bindings::nft_expr) -> i32 {
...
#[repr(C)]
struct NftPrconePriv { _flags: u32, _sreg_proto_min: u8, _sreg_proto_max: u8 }

fn nft_prcone_ipv4_ops() -> bindings::nft_expr_ops {
    bindings::nft_expr_ops {
        size: core::mem::size_of::<NftPrconePriv>() as u32,
        eval: Some(nft_prcone_ipv4_eval),
        init: Some(nft_prcone_init),
        destroy: Some(nft_prcone_destroy),
        dump: Some(nft_prcone_dump),
        validate: Some(nft_prcone_validate),
